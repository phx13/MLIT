...
class Base64Helper:
    ...
    def picture_to_base64(image_name):
        try:
            with open('EDAP/resources/images/' + image_name, 'rb') as f:
    ...
    def base64_to_picture(base64_data, image_name):
        if base64_data.startswith("data:image/"):
            is_existed = os.path.exists('EDAP/resources/images')
            if not is_existed:
                os.mkdir('EDAP/resources/images')
            img_data = base64.b64decode(base64_data.split(",")[-1].encode("utf-8"))
            with open('EDAP/resources/images/' + image_name, 'wb') as f:
                f.write(img_data)
            return image_name
        else:
            raise Exception
